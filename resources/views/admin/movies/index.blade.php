<x-layouts.stream :title="'Admin Movies - AroStream'">
    @include('admin.partials.nav')

    <section class="mt-5 rounded-xl border border-white/10 bg-slate-900/70 p-4">
        <div class="mb-4 flex items-center justify-between">
            <h1 class="text-lg font-semibold">Movies</h1>
            <a href="{{ route('admin.movies.create') }}" class="rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white">Add Movie</a>
        </div>
            ...
            @foreach ($movies as $movie)
                <div class="flex items-center justify-between rounded-md border border-white/10 bg-slate-950/60 px-3 py-2">
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $movie->language?->name }} · {{ $movie->vj?->name }} · {{ ucfirst($movie->status) }}</p>
                    </div>
