    ...
    <div class="min-h-screen lg:grid lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside class="border-b border-white/10 bg-slate-900/80 p-4 backdrop-blur lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
            ...
            <nav class="space-y-1 text-sm">
                ...
                @endcan
                @can('view-reports')
                    <a href="{{ route('admin.reports.index') }}" class="block rounded-md px-3 py-2 {{ request()->routeIs('admin.reports.*') ? 'bg-white/15 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">Reports</a>
                @endcan
            </nav>
